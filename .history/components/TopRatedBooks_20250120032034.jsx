...
const TopRatedBooks = ({ handleClick }) => {
  ...
  useEffect(() => {
    ...
    // Lấy danh sách sách có đánh giá cao nhất
    const fetchTopRatedBooks = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          // Lọc sách có `rating` và sắp xếp theo `rating` giảm dần
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        <Text style={styles.title}>Sách Đánh Giá Cao</Text>
        <Pressable onPress={() => router.push('/TopRatedBooks')}>
