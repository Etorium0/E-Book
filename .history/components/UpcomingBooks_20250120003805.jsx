...
import { database } from '../backend/firebase/FirebaseConfig'; // Import Firebase database (giả sử bạn đã cấu hình)
    ...
    const fetchUpcomingBooks = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          const upcomingBooks = books.filter(book => new Date(book.created_at) > new Date());  // Điều kiện lọc sách sắp ra mắt
              ...
              <View style={styles.bookContainer}>
                ...
                <View style={styles.infoContainer}>
                  ...
                  <Text style={styles.bookAuthor} numberOfLines={1}>
                    {item.authors ? Object.values(item.authors).map((authorId) => authorId.name).join(", ") : 'Tác giả'}
                  </Text>
