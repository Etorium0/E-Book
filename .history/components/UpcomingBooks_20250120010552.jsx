...
const UpcomingBooks = ({ handleClick }) => {
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      // In ra tất cả các sách và giá trị `createdat` để kiểm tra
      ...
      const upcomingBooks = books.filter(book => {
        ...
        // Kiểm tra nếu `createdat` là chuỗi ngày hợp lệ
        ...
        // So sánh ngày
        const isUpcoming = new Date(createdAt) > new Date();
        console.log(`Is book ${book.name} upcoming?`, isUpcoming);
        return isUpcoming;
      });

      console.log('Upcoming books:', upcomingBooks); // In ra sách đã lọc
